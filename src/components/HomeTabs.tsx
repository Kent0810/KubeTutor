...
const commonProblems = [
  // Kubernetes
  {
    ...
    cause:
      "Container keeps crashing on startup — bad entrypoint, missing env var, or a failing health check.",
  ...
  },
  {
    ...
    cause:
      "Scheduler can't place the pod — not enough CPU/memory or no node matches the selector/taint.",
  ...
  },
  {
    ...
    cause:
      "Selector mismatch between Service and Pod labels, wrong targetPort, or the pod isn't Ready.",
  ...
  },
  {
    ...
    cause:
      "Kubelet can't pull the image — wrong tag, private registry, or network issue on the node.",
  ...
  },
  {
    ...
    cause:
      "Finalizers are blocking deletion, or the node is unreachable and the pod can't be evicted.",
  ...
  },
  {
    ...
    cause:
      "Pod references a ConfigMap or Secret that doesn't exist or is in a different namespace.",
  ...
  },
  {
    ...
    cause:
      "CoreDNS pod is unhealthy, or a NetworkPolicy is blocking traffic to kube-dns on port 53.",
  ...
  },
  {
    ...
    cause:
      "Init container exits non-zero — often a missing dep, wrong command, or network not ready yet.",
  ...
  },
  {
    ...
    cause:
      "A COPY of frequently-changing files (package.json, .env) invalidates all subsequent layers.",
  ...
  },
  {
    ...
    cause:
      "Containers are on different networks, or you're using container name that isn't the DNS alias.",
  ...
  return (
    ...
    >
      ...
      >
        {/* Gradient header */}
        <div
          className={`bg-gradient-to-br ${tip.grad} flex flex-col items-center gap-2 px-6 pt-8 pb-6`}
        >
          <div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-white/20 text-4xl shadow-inner">
            {tip.icon}
          </div>
          <h3 className="mt-1 text-center text-lg font-extrabold text-white drop-shadow">
            {tip.title}
          </h3>
          ...
          <div className="space-y-1">
            {tip.detail.split("\n").map((line, i) => {
              ...
              return isCode ? (
                <code
                  key={i}
                  className="block rounded bg-slate-100 px-2 py-0.5 font-mono text-xs whitespace-pre text-slate-800"
                >
                  {line}
                </code>
              ) : (
            ...
            <div ref={searchRef} className="relative mb-6">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full rounded-2xl border border-slate-200 bg-white py-3 pr-10 pl-11 text-sm text-slate-900 shadow-sm transition outline-none placeholder:text-slate-400 focus:border-slate-400 focus:ring-2 focus:ring-slate-200"
                />
                {query && (
                  <button
                    onClick={() => {
              ...
              {showSuggestions && suggestions.length > 0 && (
                <div className="absolute z-20 mt-1.5 w-full overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-xl">
                  {suggestions.map((s) => (
                    <button
                      key={s.label}
